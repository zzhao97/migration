% This code is for quantifying whole squeezed nucleus
% to quantify: 
...
Tdata=table(collapse, H2B_cov_array,H2B_mat_b_array,area_b_array,'VariableNames',{'Filename','CoV','MaterialFull','AreaFull'}) %generate output table
writetable(Tdata,[directory 'H2B_full_output_data.csv'])
...
H2B_img_orig=zeros([size(imread(filename),1) size(imread(filename),2) nframes]);
