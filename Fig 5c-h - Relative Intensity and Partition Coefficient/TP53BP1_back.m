...
Tdata=table(collapse, TP53BP1_cov_array,TP53BP1_mat_b_array,area_b_array,'VariableNames',{'Filename','CoV','MaterialBack','AreaBack'}) %generate output table
writetable(Tdata,[directory '20231123_no_squeeze_back_output_data.csv'])
...
TP53BP1_img_orig=zeros([size(imread(filename),1)+4 size(imread(filename),2) nframes]);
...
%read in multistack
for k=1:nframes
    TP53BP1_img_orig(5:end,:,k)=imread(filename,3*k-1);
    H2B_img_orig(5:end,:,k)=imread(filename,3*k-2);
...
%     disp(RNPS1_mat_b(k,1));
    ...
    mask_nucleoli(:,:,k) = bwareafilt(imcomplement(bwareafilt(imbinarize(TP53BP1_filt(:,:,k),nucl_threshmat(:,:,k)),[8e2 5e4])),[200,3800]);
