% This code is for quantifying the whole squeezed nucleus
...
% area_b: area of the whole nucleus
%
...
dirobj=dir([input_directory '*whole.tif']);%find all tifs in the directory
fnames={};
...
for f = 1:numfiles
    filename = fnames{f};
    [mat_b_temp, area_b_temp, nframes] = getFullMeasurementsSingle(filename);
...
writetable(Tdata,[input_directory 'BRD4_full_output_data_20240306.csv'])


%% EYFP only cells (lacks H2B-miRFP as nucleus marker)
function [mat_b, area_b, nframes] = getFullMeasurementsSingle(filename)
...
BRD4_img_orig=zeros([size(imread(filename),1) size(imread(filename),2) nframes]);
BRD4_img=BRD4_img_orig;


%read in multistack
for k=1:nframes
    BRD4_img_orig(:,:,k)=imread(filename,2*k-1);
...
for k=1:nframes
    ...
    mask_nuc(:,:,k)=imclearborder(bwareafilt(imfill(imbinarize(BRD4_filt(:,:,k),thresh),'holes'),[7e3 9e4]));

    nucl_threshmat(:,:,k)=imfilter(BRD4_filt(:,:,k),fspecial('disk',60));
    mask_nucleoli(:,:,k) = bwareafilt(imcomplement(bwareafilt(imbinarize(BRD4_filt(:,:,k),nucl_threshmat(:,:,k)),[1e3 9e4])),[100,3800]);
    mask_adjusted(:,:,k) = mask_nuc(:,:,k).*(~mask_nucleoli(:,:,k));
    mask_adjusted(:,:,k) = logical(imclearborder(imerode(mask_adjusted(:,:,k),se)));
    local_mask(:,:,k) = bwareafilt(logical(mask_adjusted(:,:,k)), [8e2, 9e4]);
    ...
    % end
    % Image = getframe(gcf);
    % %imwrite(Image.cdata, sprintf('DDX4_mask_%s_frame%d.png', filename, k));
    % %close
