...
for f = 1:numfiles
    ...
    if contains(filename, 'only')
        [cov_temp, mat_b_temp, area_b_temp, nframes] = getFullMeasurementsSingle(filename);
    else
...
Tdata=table(collapse, EYFP_cov_array,EYFP_mat_b_array,area_b_array,'VariableNames',{'Filename','CoV','MaterialFull','AreaFull'}) %generate output table
writetable(Tdata,[directory 'EYFP_only_full_output_data.csv'])


%% EYFP only cells (lacks H2B-miRFP as nucleus marker)
function [cov, mat_b, area_b, nframes] = getFullMeasurementsSingle(filename)
...
%     disp(EYFP_mat_b(k,1));
    ...
    mask_nucleoli(:,:,k) = bwareafilt(imcomplement(bwareafilt(imbinarize(EYFP_filt(:,:,k),nucl_threshmat(:,:,k)),[1e3 5e4])),[200,3800]);
